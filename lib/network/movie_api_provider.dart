...
class MovieApiProvider {
  ...
  Future<MovieCategoryModel> fetchMovieCategoryList(String type) async {
    final response = await client
        .get("http://api.themoviedb.org/3/genre/movie/$type?api_key=$_apiKey");
    print(response.request.url);
    print(response.body.toString());
    if (response.statusCode == 200) {
      return MovieCategoryModel.fromJson(json.decode(response.body));
    } else {
      throw Exception('Failed to load post');
    }
  }
